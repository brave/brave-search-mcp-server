...
export const params = z.object({
  ...
  search_lang: z
    .preprocess(
      // Brave's API uses `jp`, but agents usually send ISO `ja`.
      (value) => (typeof value === 'string' && value.toLowerCase() === 'ja' ? 'jp' : value),
      z
        .enum([
          'ar',
          'eu',
        ...
        .default('en')
        .describe(
          'Search language preference. The 2 or more character language code for which the search results are provided. Brave uses `jp` for Japanese (not ISO `ja`); callers that pass `ja` are remapped.'
        )
    )
    .optional(),
  ui_lang: z
